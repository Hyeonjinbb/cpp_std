...
int main() {
    ...
    monster_c monster_c_obj1, monster_c_obj2; //두 몬스터 객체 생성
    monster_c_obj2.set_level(2); //두번째 몬스터 레벨 설정
    player player1; //플레이어 객체 생성
    monster_c new_monster_c_obj = monster_c_obj1 + monster_c_obj2; //몬스터 두마리 합체

    cout << "Player 합체 전 몬스터C HP[" << new_monster_c_obj.get_hp() //합체 전 몬스터 체력 출력
        << "]" << endl;

    new_monster_c_obj = new_monster_c_obj + player1; //플레이어와 몬스터 합체

    cout << "Player 합체 후 몬스터C HP[" << new_monster_c_obj.get_hp() //합체 후 몬스터 체력 출력
        << "]" << endl;
